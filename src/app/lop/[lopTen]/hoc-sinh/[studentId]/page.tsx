...
export default function StudentDetailPage() {
  ...
  useEffect(() => {
    ...
    loadData();
    setIsEditing(false); // Reset mode khi đổi học sinh
  }, [studentId, lopTen]);
  ...
  const handleSaveAll = async () => {
    ...
    try {
      ...
      for (const key in draftData) {
        if (draftData[key] !== student[key]) {
          updates[key] = draftData[key];
          changes.push({
            field: key,
            old: student[key] || "trống",
            new: draftData[key] || "trống"
          });
        }
      }
  ...
  return (
    <div className="flex flex-col md:flex-row gap-0 h-[calc(100vh-100px)] -mx-4 -mt-6 overflow-hidden bg-white">
      {toast && (
        ...
        <div className="flex-1 overflow-y-auto p-1 space-y-0.5 custom-scrollbar">
          {classList.filter(s => (s.hoTen || "").toLowerCase().includes(search.toLowerCase())).map(s => (
            <Link key={s.id} href={`/lop/${encodeURIComponent(lopTen)}/hoc-sinh/${s.id}`} className={`flex items-center gap-2 px-3 py-2.5 rounded-xl transition-all ${s.id === studentId ? 'bg-blue-600 text-white shadow-lg' : 'hover:bg-slate-50 text-slate-600'}`}>
              <p className="font-bold text-[11px] truncate uppercase">{s.hoTen}</p>
            </Link>
          ))}
        </div>
      </div>
          ...
          <>
            ...
            <div className="bg-white p-4 border-b border-slate-200 flex justify-between items-center shadow-sm">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-lg font-black uppercase text-slate-800 tracking-tight">{student.hoTen}</h2>
            ...
            <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
              {/* CỘT GIỮA: CÁC Ô KẾT QUẢ */}
              <div className="flex-1 p-4 overflow-y-auto custom-scrollbar">
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                  {GRADES.map(yr => (
                    <div key={yr} className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                      <div className="bg-slate-800 text-white px-3 py-2 text-[11px] font-black uppercase tracking-widest text-center">LỚP {yr}</div>
                      <div className="grid grid-cols-3 border-collapse">
                        ...
                        <div className="border-r border-slate-100">
                          <div className="h-10 bg-slate-50 border-b border-slate-200 flex items-center justify-center text-[9px] font-black text-slate-400 text-center uppercase leading-none">Kết quả<br/>rèn luyện</div>
                          {PERIODS.map(p => (
                            <div key={p.key} className="h-10 border-b border-slate-100 last:border-0 flex items-center justify-center px-1">
                              {isEditing ? (
                                <input value={draftData[`kq_rl_${yr}_${p.key}`] || ""} onChange={e => setDraftData({...draftData, [`kq_rl_${yr}_${p.key}`]: e.target.value})} className="w-full text-center py-1 bg-blue-50 border border-blue-200 rounded font-black text-xs outline-none" />
                              ) : (
                                <span className={`text-xs ${XEPLOAI_STYLE(student[`kq_rl_${yr}_${p.key}`])}`}>{student[`kq_rl_${yr}_${p.key}`] || "—"}</span>
                              )}
                            </div>
                          ))}
                        </div>
                        <div>
                          <div className="h-10 bg-slate-50 border-b border-slate-200 flex items-center justify-center text-[9px] font-black text-slate-400 text-center uppercase leading-none">Kết quả<br/>học tập</div>
                          {PERIODS.map(p => (
                            <div key={p.key} className="h-10 border-b border-slate-100 last:border-0 flex items-center justify-center px-1">
                              {isEditing ? (
                                <input value={draftData[`kq_ht_${yr}_${p.key}`] || ""} onChange={e => setDraftData({...draftData, [`kq_ht_${yr}_${p.key}`]: e.target.value})} className="w-full text-center py-1 bg-blue-50 border border-blue-200 rounded font-black text-xs outline-none" />
                              ) : (
                                <span className={`text-xs ${XEPLOAI_STYLE(student[`kq_ht_${yr}_${p.key}`])}`}>{student[`kq_ht_${yr}_${p.key}`] || "—"}</span>
                              )}
                            </div>
                          ))}
                        </div>
                ...
                {/* Chân trang bảng giữa */}
                <div className="mt-4 grid grid-cols-2 gap-4">
                  <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex justify-between items-center">
                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Xếp loại tốt nghiệp</span>
                    {isEditing ? (
                      <input value={draftData.xep_loai_tot_nghiep || ""} onChange={e => setDraftData({...draftData, xep_loai_tot_nghiep: e.target.value})} className="w-24 text-center py-1.5 bg-blue-50 border border-blue-200 rounded-xl font-black text-xs outline-none" />
                    ) : (
                      <span className="text-sm font-black text-blue-600 uppercase">{student.xep_loai_tot_nghiep || "—"}</span>
                    )}
                  </div>
                  <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex justify-between items-center">
                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">ĐTB Lớp 9</span>
                    {isEditing ? (
                      <input value={draftData.tong_diem_9_cn || ""} onChange={e => setDraftData({...draftData, tong_diem_9_cn: e.target.value})} className="w-24 text-center py-1.5 bg-blue-50 border border-blue-200 rounded-xl font-black text-xs outline-none" />
                    ) : (
                      ...
                      {SUBJECTS_ORDER.map(({ key, label }) => (
                        <tr key={key} className="hover:bg-slate-50 transition-colors">
                          ...
                          <td className="p-2 text-center">
                            {isEditing ? (
                              <input value={draftData[key] || ""} onChange={e => setDraftData({...draftData, [key]: e.target.value})} className="w-full text-center py-1 bg-blue-50 border border-blue-200 rounded font-black text-xs outline-none" />
                            ) : (
                              <span className="font-black text-blue-600">{student[key] || "—"}</span>
                            )}
                          </td>
                        </tr>
                      ))}
